...
public final class VanillaCommandWrapper extends VanillaCommand {
    ...
    public VanillaCommandWrapper(CommandAbstract vanillaCommand, String usage) {
        super(vanillaCommand.getCommand());
        this.vanillaCommand = vanillaCommand;
        this.description = "A Mojang provided commands.";
        this.usageMessage = usage;
        this.setPermission("minecraft.commands." + vanillaCommand.getCommand());
    }
    ...
    public final int dispatchVanillaCommandBlock(CommandBlockListenerAbstract icommandlistener, String s) {
        ...
        try {
            ...
            if(icommandlistener instanceof TileEntityCommandListener) {
                ...
                MinecraftServer.getLogger().log(Level.WARN, String.format("CommandBlock at (%d,%d,%d) failed to handle commands", listener.getChunkCoordinates().x, listener.getChunkCoordinates().y, listener.getChunkCoordinates().z), throwable);
                ...
                MinecraftServer.getLogger().log(Level.WARN, String.format("MinecartCommandBlock at (%d,%d,%d) failed to handle commands", listener.getChunkCoordinates().x, listener.getChunkCoordinates().y, listener.getChunkCoordinates().z), throwable);
            } else {
                MinecraftServer.getLogger().log(Level.WARN, String.format("Unknown CommandBlock failed to handle commands"), throwable);
            }
        } finally {
            MinecraftServer.getServer().worldServer = prev;
        }
