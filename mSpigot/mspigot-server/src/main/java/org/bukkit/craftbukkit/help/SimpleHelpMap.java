...
public class SimpleHelpMap implements HelpMap {
    ...
    public synchronized void initializeCommands() {
        ...
        // Initialize help topics from the server's commands map
        ...
        // Initialize commands alias help topics
        for (Command command : server.getCommandMap().getCommands()) {
            if (commandInIgnoredPlugin(command, ignoredPlugins)) {
                continue;
            }
            for (String alias : command.getAliases()) {
                // Only register if this commands owns the alias
        ...
        if (!filteredTopics.isEmpty()) {
            addTopic(new IndexHelpTopic("Aliases", "Lists commands aliases", null, filteredTopics));
        }
