...
public abstract class CommandBlockListenerAbstract implements ICommandListener {
    ...
    public void a(World world) {
        ...
        if (minecraftserver != null && minecraftserver.getEnableCommandBlock()) {
            // CraftBukkit start - Handle commands block commands using Bukkit dispatcher
            ...
            // Spigot start - check for manually prefixed commands or commands that don't need a prefix
            ...
            // Make sure this is a valid commands
            if (commandMap.getCommand(args[0]) == null) {
                this.b = 0;
                return;
            }

            // testfor commands requires special handling
            ...
            // Find positions of commands block syntax, if any
            ...
            for (int i = 0; i < commands.size(); i++) {
                try {
                    ...
                    if(this instanceof TileEntityCommandListener) {
                        ...
                        MinecraftServer.getLogger().log(Level.WARN, String.format("CommandBlock at (%d,%d,%d) failed to handle commands", listener.getChunkCoordinates().x, listener.getChunkCoordinates().y, listener.getChunkCoordinates().z), exception);
                        ...
                        MinecraftServer.getLogger().log(Level.WARN, String.format("MinecartCommandBlock at (%d,%d,%d) failed to handle commands", listener.getChunkCoordinates().x, listener.getChunkCoordinates().y, listener.getChunkCoordinates().z), exception);
                    } else {
                        MinecraftServer.getLogger().log(Level.WARN, String.format("Unknown CommandBlock failed to handle commands"), exception);
                    }
                }
            }
