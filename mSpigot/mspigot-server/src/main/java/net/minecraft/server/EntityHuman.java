...
public abstract class EntityHuman extends EntityLiving implements ICommandListener {
    ...
    public void attack(final Entity entity) {
        if (entity.av() && !entity.j(this)) {
            ...
            if (f > 0.0f || f2 > 0.0f) {
                final boolean flag = !this.world.paperSpigotConfig.disablePlayerCrits && this.fallDistance > 0.0f && !this.onGround && !this.h_() && !this.M() && !this.hasEffect(MobEffectList.BLINDNESS) && this.vehicle == null && entity instanceof EntityLiving;
                if (flag && f > 0.0f) {
                    f *= 1.5f;
                }
                f += f2;
                boolean flag2 = false;
                final int j = EnchantmentManager.getFireAspectEnchantmentLevel(this);
                if (entity instanceof EntityLiving && j > 0 && !entity.isBurning()) {
                    final EntityCombustByEntityEvent combustEvent = new EntityCombustByEntityEvent(this.getBukkitEntity(), entity.getBukkitEntity(), 1);
                ...
                final boolean flag3 = entity.damageEntity(DamageSource.playerAttack(this), f);

                if (flag3) {
                    Knockback profile = (this.getKnockback() == null) ? SpigotConfig.globalKbProfile : this.getKnockback();
                    if (i > 0) {
                        if (profile.isKohi()) {
                            entity.g((-MathHelper.sin(this.yaw * 3.1415927F / 180.0F) * i * 0.5F), 0.1D, (MathHelper.cos(this.yaw * 3.1415927F / 180.0F) * i * 0.5F));
                        } else {
                            entity.g((-MathHelper.sin(this.yaw * 3.1415927F / 180.0F) * (float) i * profile.getExtraHorizontal()), profile.getExtraVertical(), (MathHelper.cos(this.yaw * 3.1415927F / 180.0F) * (float) i * profile.getExtraHorizontal()));
                        }

                        this.motX *= 0.6D;
                        this.motZ *= 0.6D;

                        if (!profile.isAutoWTap()) {
                    ...
                    final ItemStack itemstack = this.bF();
                    ...
                    if (entity instanceof EntityLiving) {
                        ...
                        if (j > 0) {
                            final EntityCombustByEntityEvent combustEvent2 = new EntityCombustByEntityEvent(this.getBukkitEntity(), entity.getBukkitEntity(), j * 4);
