...
import net.lugami.practice.util.MongoUtils;
 ...
 * have elo, etc.
 */
// This class purposely uses stark Gson (as we want to actually serialize
...
public final class KitType {
    ...
    private static final MongoCollection<Document> collection = MongoUtils.getCollection(MONGO_COLLECTION_NAME);
    @Getter public static final List<KitType> allTypes = new ArrayList<>();
    public static KitType teamFight = new KitType();
    static {
        collection.find().iterator().forEachRemaining(doc -> allTypes.add(qLib.PLAIN_GSON.fromJson(doc.toJson(), KitType.class)));
    ...
    @Getter @Setter private String knockbackName = null;
    ...
    public boolean isSumo() {
        return this.getId().equalsIgnoreCase("SUMO");
    }
