...
public class Tournament {
    ...
    private final Map<String, Map<Integer, Tournament>> requiredTeams = new HashMap<>();
    ...
    public int countTeamSize(Tournament tournament) {
        Map<Integer, Tournament> requiredTeam = requiredTeams.get(tournament.getType().getDisplayName());
        return requiredTeam != null ? requiredTeam.size() : 128;
    }
    ...
    public void check() {
        ...
        if (activeParties.size() == 1) {
            ...
            if (party.getMembers().size() == 1) {
                repeatMessage(ChatColor.translateAlternateColorCodes('&', "&6" + PatchedPlayerUtils.getFormattedName(party.getLeader()) + " &7won the tournament!"), 4, 2);
                ...
                repeatMessage(ChatColor.translateAlternateColorCodes('&', "&6" + PatchedPlayerUtils.getFormattedName(members[0]) + " &7and &6" + PatchedPlayerUtils.getFormattedName(members[1]) + " &7won the tournament!"), 4, 2);
            } else {
                repeatMessage(ChatColor.translateAlternateColorCodes('&', "&6" + PatchedPlayerUtils.getFormattedName(party.getLeader()) + "&7's team won the tournament!"), 4, 2);
            }
    ...
    private void leftTournament(Party party) {
        broadcastJoinMessage(party);
    }
    ...
    private void startRound() {
        ...
        Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&', "&7Use &6/status &7 see who is fighting."));
    ...
    private void broadcastJoinMessage(Party joiningParty) {
        if (getCurrentRound() != -1) {
            ...
            if (joiningParty.getMembers().size() == 1) {
                message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&c&lDONOR ONLY &7- " + PatchedPlayerUtils.getFormattedName(joiningParty.getLeader()) + "&7 &7has &7joined &7the &6tournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
            } else if (joiningParty.getMembers().size() == 2) {
                Iterator<UUID> membersIterator = joiningParty.getMembers().iterator();
                message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&c&lDONOR ONLY &7- " + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 &7and &6" + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 have joined the &6tournament&7. &7(" + activeParties.size() * 2 + "/" + requiredPartiesToStart * 2 + "&7)"));
            } else {
                message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&c&lDONOR ONLY &7- " + PatchedPlayerUtils.getFormattedName(joiningParty.getLeader()) + "&7's team has joined the &6tournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
            }

            message.tooltip(ChatColor.translateAlternateColorCodes('&', "&6Donators &7can join during the tournament countdown. Purchase a rank at &d " + (Practice.getInstance().getDominantColor() == ChatColor.LIGHT_PURPLE ? "store." + Practice.getInstance().getNetworkWebsite() : "store." + Practice.getInstance().getNetworkWebsite()) +  " &7."));
        ...
        if (joiningParty.getMembers().size() == 1) {
            ...
            Iterator<UUID> membersIterator = joiningParty.getMembers().iterator();
            message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&6" + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 and &7" + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 have joined the &6tournament&7. &7(" + activeParties.size() * 2 + "/" + requiredPartiesToStart * 2 + "&7)"));
        } else {
    ...
    private void broadcastLeftMessage(Party joiningParty) {
        if (getCurrentRound() != -1) {
            // donor join
            FancyMessage message;
            if (joiningParty.getMembers().size() == 1) {
                message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&c&lDONOR ONLY &7- " + PatchedPlayerUtils.getFormattedName(joiningParty.getLeader()) + "&7 &7has &7left &7the &6tournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
            } else if (joiningParty.getMembers().size() == 2) {
                Iterator<UUID> membersIterator = joiningParty.getMembers().iterator();
                message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&c&lDONOR ONLY &7- " + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 &7and &6" + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 have left the &6tournament&7. &7(" + activeParties.size() * 2 + "/" + requiredPartiesToStart * 2 + "&7)"));
            } else {
                message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&c&lDONOR ONLY &7- " + PatchedPlayerUtils.getFormattedName(joiningParty.getLeader()) + "&7's team has left the &6tournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
            }
        ...
        FancyMessage message;
        if (joiningParty.getMembers().size() == 1) {
            message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&6" + PatchedPlayerUtils.getFormattedName(joiningParty.getLeader()) + "&7 has left the &6tournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
        } else if (joiningParty.getMembers().size() == 2) {
            Iterator<UUID> membersIterator = joiningParty.getMembers().iterator();
            message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&6" + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 and &7" + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 have left the &6tournament&7. &7(" + activeParties.size() * 2 + "/" + requiredPartiesToStart * 2 + "&7)"));
        } else {
            message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&6" + PatchedPlayerUtils.getFormattedName(joiningParty.getLeader()) + "&7's team has left the &6tournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
        }
    ...
    private void broadcastEliminationMessage(Party loserParty) {
        ...
        if (loserParty.getMembers().size() == 1) {
            message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&c" + PatchedPlayerUtils.getFormattedName(loserParty.getLeader()) + "&7 has been eliminated. &7(" + activeParties.size() * multiplier + "/" + requiredPartiesToStart * multiplier + "&7)"));
        } else if (loserParty.getMembers().size() == 2) {
            Iterator<UUID> membersIterator = loserParty.getMembers().iterator();
            message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&c" + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 and &6" + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + " &7were eliminated. &7(" + activeParties.size() * multiplier + "/" + requiredPartiesToStart * multiplier + "&7)"));
        } else {
            message = new FancyMessage(ChatColor.translateAlternateColorCodes('&', "&c" + PatchedPlayerUtils.getFormattedName(loserParty.getLeader()) + "&7's team has been eliminated. &7(" + activeParties.size() * multiplier + "/" + requiredPartiesToStart * multiplier + "&7)"));
        }

        message.command("/dem");
        message.tooltip(ChatColor.translateAlternateColorCodes('&', "&6&lCLICK &7 hide this message."));
        SettingHandler settingHandler = Practice.getInstance().getSettingHandler();
