...
import net.lugami.practice.util.AngleUtils;
 ...
 * See {@link net.lugami.practice.arena} for a comparision of
...
public final class Arena {
    ...
    @Getter
    @Setter(AccessLevel.PACKAGE) private transient boolean inUse;
    ...
    public void restore() {
        synchronized (chunkSnapshots) {
            World world = bounds.getWorld();
            chunkSnapshots.entrySet().forEach(entry -> world.getChunkAt(LongHash.msw(entry.getKey()), LongHash.lsw(entry.getKey())).restoreSnapshot((ChunkSnapshot) entry.getValue()));
